...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/02 12:02:49 by eguler            #+#    #+#             */
/*   Updated: 2022/02/05 14:29:45 by eguler           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*read_fr_fd;
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	read_fr_fd = ft_read(read_fr_fd, fd);
	if (!read_fr_fd)
		return (NULL);
	line = get_line(read_fr_fd);
	read_fr_fd = del_line(read_fr_fd);
	return (line);
}
/*
int main()
